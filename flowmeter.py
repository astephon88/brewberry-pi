...
class FlowMeter():
  ...
  def __init__(self, displayFormat, beverage,flowConstant):
    self.displayFormat = displayFormat
    self.beverage = beverage
    self.flowConstant=flowConstant
    self.clicks = 0
  ...
  def update(self, currentTime):
    ...
    if (self.enabled == True and self.clickDelta < 1000):
      ...
      if (self.hertz > FlowMeter.MIN_HZ and self.hertz < FlowMeter.MAX_HZ): # Only update the flow is fast enough, to avoid electrical shenanigans by the flow meters.
        self.flow = self.hertz / (FlowMeter.SECONDS_IN_A_MINUTE *self.flowConstant)   #In Liters per second
